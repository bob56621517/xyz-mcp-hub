 ...
 *
 * <p>元素定位（target）优先按 CSS 选择器，其次按页面可见文本。所有工具返回文本；
 * 截图以 {@code data:image/*;base64,...} 形式返回。浏览器会话由 {@link PlaywrightSession}
 * 管理，首次调用时懒启动无头 chromium。</p>
 */
@Component
public class PlaywrightTools {
	...
	public PlaywrightTools(PlaywrightSession session) {
		this.session = session;
	}

	@Tool(name = "browser_navigate", description = "导航到指定 URL，等待页面加载完成。url 必填。")
	public String browserNavigate(@ToolParam(description = "要导航到的完整 URL，如 https://example.com") String url) {
		try {
			var response = session.navigate(url);
	...
	@Tool(name = "browser_go_back", description = "返回浏览器历史中的上一页。")
	public String browserGoBack() {
		try {
			return session.goBack() ? "已返回上一页。" : "没有可返回的历史记录。";
		}
		catch (PlaywrightException | IllegalArgumentException e) {
	...
	@Tool(name = "browser_go_forward", description = "前进到浏览器历史中的下一页。")
	public String browserGoForward() {
		try {
			return session.goForward() ? "已前进到下一页。" : "没有可前进的历史记录。";
		}
		catch (PlaywrightException | IllegalArgumentException e) {
	...
	@Tool(name = "browser_snapshot", description = "捕获当前页面结构的可访问性快照（accessibility tree），用于理解页面内容与可交互元素。")
	...
	@Tool(name = "browser_take_screenshot", description = "对当前页面截图，返回 base64 data URL。可截取整页或指定元素，支持 png/jpeg/webp。")
	public String browserTakeScreenshot(
		...
		try {
			String base64 = session.screenshot(fullPage, selector, type);
	...
	@Tool(name = "browser_click", description = "点击页面元素。target 为 CSS 选择器或页面可见文本。")
	public String browserClick(
		...
		try {
			...
			session.locator(target).click(options);
			return "已点击 " + target + "。";
		}
	...
	@Tool(name = "browser_hover", description = "悬停在页面元素上。target 为 CSS 选择器或页面可见文本。")
	public String browserHover(@ToolParam(description = "目标元素，CSS 选择器或可见文本") String target) {
		try {
			session.locator(target).hover();
			return "已悬停 " + target + "。";
		}
		catch (PlaywrightException | IllegalArgumentException e) {
	...
	@Tool(name = "browser_type", description = "向可编辑元素输入文本。target 为 CSS 选择器或可见文本；submit 为 true 时输入后按 Enter。")
	public String browserType(
		...
		try {
			session.locator(target).fill(text);
			if (Boolean.TRUE.equals(submit)) {
				session.locator(target).press("Enter");
			}
	...
	@Tool(name = "browser_press_key", description = "在当前页面按下键盘键，如 Enter、ArrowDown、Tab、a、F5。")
	public String browserPressKey(@ToolParam(description = "按键名，如 Enter、ArrowDown") String key) {
		try {
	...
	@Tool(name = "browser_select_option", description = "在下拉框中按值选择选项。target 为 CSS 选择器或可见文本；values 为待选项的 value 列表。")
	public String browserSelectOption(
		...
		try {
			session.locator(target).selectOption(values.toArray(new String[0]));
	...
	@Tool(name = "browser_fill_form", description = "批量填充表单字段。fields 为对象数组，每项含 target（CSS 选择器或可见文本）、value、type（textbox / checkbox / radio，默认 textbox）。")
				...
				switch (type) {
					case "checkbox", "radio" -> {
						if (Boolean.parseBoolean(value)) {
							session.locator(target).check();
						}
						else {
							session.locator(target).uncheck();
						}
					}
					default -> {
						if (value == null) {
						...
						session.locator(target).fill(value);
					}
				}
				done++;
			}
	...
	@Tool(name = "browser_resize", description = "调整当前页面视口尺寸（CSS 像素）。")
	...
	@Tool(name = "browser_close", description = "关闭当前标签页。")
	public String browserClose() {
		try {
			return session.closeCurrentPage();
		}
		catch (PlaywrightException | IllegalArgumentException e) {
	...
	@Tool(name = "browser_tabs", description = "管理浏览器标签页：action 为 list（列出所有标签页）/ new（新建，可传 url）/ close（关闭，可传 index）/ select（切换，传 index）。index 从 0 开始。")
	public String browserTabs(
		...
		try {
			return session.tabs(action, index, url);
		}
		catch (PlaywrightException | IllegalArgumentException e) {
	...
	@Tool(name = "browser_handle_dialog", description = "设置浏览器对话框（alert/confirm/prompt）的自动处理：accept 为 true 时自动接受（prompt 对话框可用 promptText 填默认值），false 时自动取消。")
	public String browserHandleDialog(
		...
		session.setDialogHandler(accept, promptText);
		return accept ? "对话框将自动接受。" : "对话框将自动取消。";
	}

	@Tool(name = "browser_network_requests", description = "列出页面加载以来的网络请求。filter 为 URL 正则；isStatic 为 true 时包含图片/字体/样式等静态资源。")
	public String browserNetworkRequests(
		...
		try {
			var list = session.networkRequests(filter, isStatic);
			if (list.isEmpty()) {
				return "没有匹配的网络请求。";
			}
	...
	@Tool(name = "browser_network_request", description = "返回单个网络请求的详情。index 为 browser_network_requests 返回的序号（从 1 开始）；part 为 request-headers / request-body / response-headers / response-body，缺省返回全部。")
	public String browserNetworkRequest(
		...
		Map<String, Object> r = session.networkRequest(index);
		if (r == null) {
	...
	@Tool(name = "browser_console_messages", description = "列出页面加载以来的控制台消息。level 为 error / warning / info / debug，返回该级别及更严重级别；all 为 true 时忽略级别返回全部。")
	public String browserConsoleMessages(
		...
		var list = all != null && all ? session.consoleMessages("debug") : session.consoleMessages(level);
		if (list.isEmpty()) {
			return "没有控制台消息。";
		}
	...
	@Tool(name = "browser_evaluate", description = "在当前页面执行 JavaScript 表达式或函数，返回求值结果。function 为如 \"document.title\" 或 \"() => document.title\" 的表达式。")
	public String browserEvaluate(
			@ToolParam(description = "要执行的 JS 表达式或函数") String function) {
		try {
			Object result = session.evaluate(function);
	...
	@Tool(name = "browser_wait_for", description = "等待页面出现/消失指定文本，或等待固定时间。text 出现、textGone 消失、time（秒）等待时长，三选一。")
	public String browserWaitFor(
		...
		try {
			if (text != null && !text.isBlank()) {
				session.waitForText(text);
				return "等待到文本出现：" + text;
			}
			if (textGone != null && !textGone.isBlank()) {
				session.waitForTextGone(textGone);
				return "等待到文本消失：" + textGone;
			}
			if (time != null) {
				session.waitForTimeout(time);
				return "已等待 " + time + " 秒。";
			}
	...
	@Tool(name = "browser_find", description = "在页面可访问性快照中查找包含指定文本的行。text 为大小写不敏感子串；regex 为正则表达式，二选一。")
	public String browserFind(
		...
		try {
			String snapshot = session.snapshot();
			StringBuilder sb = new StringBuilder();
	...
	@Tool(name = "browser_file_upload", description = "向文件上传元素上传本地文件。target 为 input 的 CSS 选择器或可见文本；paths 为本地文件绝对路径列表。")
	public String browserFileUpload(
		...
		try {
			session.setInputFiles(target, paths);
