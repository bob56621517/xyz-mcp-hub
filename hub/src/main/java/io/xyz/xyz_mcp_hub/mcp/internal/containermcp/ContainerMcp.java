...
/**
 * 容器 MCP 源基类（ADR-0009 四类 MCP 之一，#37 mcp 型）：本地容器按需拉起 → 转发容器内 MCP 工具。
 *
 * <p>子类声明接入协议（{@link #protocol()}）：当前仅 {@code mcp} 型（如 markitdown）转发容器内 MCP
 * 工具，工具调用经 {@link ContainerMcpClient} 转发；{@code rest} 型（jina）已随 #53 提升为顶级模块
 * （{@code jina.JinaReader} + 工具类即源 {@code JinaTools}），不再经本基类。容器生命周期（首用拉起 +
 * 防重拉 + 闲置回收 + 关闭销毁）全部委托 {@code docker} 模块的 {@link ContainerManager}；工具清单由
 * 子类以静态冒烟数据声明（镜像由我们 pin，工具集与所 pin 镜像版本绑定，ADR-0011 决策 5）。</p>
 *
