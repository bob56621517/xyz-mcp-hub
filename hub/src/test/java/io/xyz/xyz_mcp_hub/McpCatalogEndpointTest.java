 ...
 * 与 ADR-0011 目录 schema 一致；冒烟断言当前已注册 native 源（utils / bocha / fetch / playwright）都在清单中。
...
class McpCatalogEndpointTest {
	...
	// 冒烟（本期）：当前已注册源为 4 个 native 源，类型均为 native（目录随后续源迁入自动增长）
	...
	void catalogListsAllRegisteredNativeSources() throws Exception {
		...
		for (JsonNode source : sources) {
			assertThat(source.get("type").asText()).isEqualTo("native");
		}
	}
