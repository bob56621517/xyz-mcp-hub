...
/**
 * bocha 顶级模块能力测试（#53，S1 能力层）：用 MockRestServiceServer 模拟博查 HTTP API，验证
 * {@link BochaClient} 的搜索调用与结果格式化（不触网、不经 MCP）。
 */
class BochaClientTest {
	...
	void webSearchFormatsResults() {
		...
		String text = client.webSearch("spring boot", 5, "oneMonth");
	...
	void aiSearchIncludesSummary() {
		...
		String text = client.aiSearch("spring boot", null, null);
	...
	void nonSuccessCodeReturnsErrorText() {
		...
		String text = client.webSearch("spring boot", null, null);
	...
	void emptyResultReturnsNotice() {
		...
		String text = client.webSearch("nothing", null, null);
